...
# robots.txt сайта запрещает параметр пагинации Bitrix (PAGEN_1) — проверка теперь
# делается автоматически в BaseScraper.can_fetch(), а не «помнится» по комментарию.
# Пока пагинация запрещена, полную глубину категории обойти нельзя, поэтому обходим
# МЕЛКИЕ подкатегории (а не 13 верхних разделов): первая страница каждой из них
# покрывает бОльшую долю каталога.
